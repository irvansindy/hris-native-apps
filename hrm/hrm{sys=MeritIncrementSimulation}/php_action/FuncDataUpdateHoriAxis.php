...
if ($_POST) {
	...
	$last_hori_value     	= $_POST['last_hori_value'];
	$new_hori_value			= $_POST['new_hori_value'];
	$sel_period				= $_POST['sel_period_hori'];
	$sel_comparatio			= $_POST['sel_comparatio'];
	...
	$sql = "UPDATE `hrmperf_compaaxis`
					SET
						`total_value`					= '$new_hori_value'
					WHERE 
						`total_value`					= '$last_hori_value' AND
						`axis`							= 'X'";
	...
	$sql_0 = "UPDATE `hrmperf_comparatio`
					SET
						`index_percentage_horizontal`		= '$new_hori_value'
					WHERE `ip_period` 						= '$sel_period' AND
							`index_percentage_horizontal`	= '$last_hori_value' AND
							`compa_ratio_id`				= '$sel_comparatio'";
	...
	if ($query_0 == TRUE) {
		...
		$validator['messages'] = "Successfully Update Horizontal Axis";
	} else {

		$validator['success'] = false;
		$validator['code'] = "failed_message";
		$validator['messages'] = "Failed Update Horizontal Axis";
	}
