...
$sql = "UPDATE `hrmrequestapproval` SET
					`status` 		= '1'
				WHERE
					`request_no` 		= '$sel_reject_spvup' AND
...
if($query == TRUE) {
	...
	if(mysqli_num_rows($get_any_request) < 1 ){
		...
		$sql1 = "UPDATE hrmrequestapproval SET request_status = '5' WHERE request_no = '$sel_reject_spvup'";
		$sql0 = "UPDATE hrmperf_ipprequest SET `status` = '5' WHERE ipp_reqno = '$sel_reject_spvup'";
	}
	$query = $connect->query($sql1);
	$query = $connect->query($sql0);

	$validator['success'] = true;
	$validator['code'] = "success_message_reject_spv_up";
