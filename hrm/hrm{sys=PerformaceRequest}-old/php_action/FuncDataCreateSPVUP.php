...
if($_POST) {	
	...
	$is_any_request = mysqli_query($connect, "SELECT * FROM `hrmperf_ipprequest` WHERE `ipp_reqno` = '$SFnumbercon'");
	...
	if($query_num_rows > '0') {						
		...
		$validator['messages'] = "Performance has been create before please check " . $SFnumbercon;

	} elseif($query_num_rows == '0' && $sql_1 == TRUE && $sql_2 == TRUE && $query == TRUE) {						
		$validator['success'] = true;
		$validator['code'] = "success_message_spv_up";
		$validator['messages'] = $alert_print_0;	
				
	} elseif($sql_1 == TRUE && $sql_2 == FALSE && $query == TRUE) {	
