...
if(!empty($_POST['worklocation'])){
    $work_location = json_encode($_POST['worklocation']);
    $work_location_array1 = str_replace('[', '', $work_location);
    $work_location_array2 = str_replace(']', '', $work_location_array1);
}else{
    $work_location_array2 = "''";
}

if(!empty($_POST['cost'])){
    $cost_center = json_encode($_POST['cost'], TRUE);
    $cost_center_array1 = str_replace('[', '', $cost_center);
    $cost_center_array2 = str_replace(']', '', $cost_center_array1);
}else{
    $cost_center_array2 = "''";
}

if(!empty($_POST['status'])){
    $status = json_encode($_POST['status'], TRUE);
    $status1 = $_POST['status'];
    $count_status = count($status1);
}else{
    $status = '';
    $status1 = ["HAHA","HOHO"];
    $active = '';
...
if($count_status == '1'){
    if($status1[0] == 'active'){
        $active = 'active';
        $nonactive = '';
    }elseif($status1[0] == 'nonactive'){
        $active = '';
        $nonactive = 'nonactive';
    }
}elseif($count_status == '2'){
    $active = $status1[0];
    $nonactive = $status1[1];
}

$status_array1 = str_replace('[', '', $status);
$status_array2 = str_replace(']', '', $status_array1);

// FIlter
if(empty($_POST['worklocation']) && empty($_POST['cost']) && empty($_POST['status'])){
    $where = "";
}elseif(empty($_POST['worklocation']) && empty($_POST['cost']) && !empty($_POST['status'])){
    $where = "AND a.status IN ($status_array2)";
}elseif(empty($_POST['worklocation']) && !empty($_POST['cost']) && empty($_POST['status'])){
    $where = "AND a.cost_center IN ($cost_center_array2)";
}elseif(empty($_POST['worklocation']) && !empty($_POST['cost']) && !empty($_POST['status'])){
    $where = "AND a.cost_center IN ($cost_center_array2) AND a.status IN ($status_array2)";
}elseif(!empty($_POST['worklocation']) && empty($_POST['cost']) && empty($_POST['status'])){
    $where = "AND a.plant IN ($work_location_array2)";
}elseif(!empty($_POST['worklocation']) && empty($_POST['cost']) && !empty($_POST['status'])){
    $where = "AND a.plant IN ($work_location_array2) AND a.status IN ($status_array2)";
}elseif(!empty($_POST['worklocation']) && !empty($_POST['cost']) && empty($_POST['status'])){
    $where = "AND a.plant IN ($work_location_array2) AND a.cost_center IN ($cost_center_array2)";
}elseif(!empty($_POST['worklocation']) && !empty($_POST['cost']) && !empty($_POST['status'])){
    $where = "AND a.plant IN ($work_location_array2) AND a.cost_center IN ($cost_center_array2) AND a.status IN ($status_array2)";
}
                ...
                <tr>
                    <td>
                        <select id="worklocation" name="worklocation[]" multiple class="form-control" >
                            ...
                            WHERE a.plant NOT IN ($work_location_array2)
                            GROUP BY a.plant");
                            $sql_work_location_selected = mysqli_query($connect, "SELECT 
                            a.plant
                            FROM dashboard_gen a 
                            WHERE a.plant IN ($work_location_array2)
                            GROUP BY a.plant");
                            ?>
                            <?php while($data_work_location_selected = mysqli_fetch_assoc($sql_work_location_selected)){ ?>
                            <option value="<?php echo $data_work_location_selected['plant']; ?>" selected><?php echo $data_work_location_selected['plant']; ?></option>
                            <?php } ?>
                            <?php while($data_work_location = mysqli_fetch_assoc($sql_work_location)){ ?>
                ...
                <tr>
                    <td>
                        <select id="cost" name="cost[]" multiple class="form-control" >
                            ...
                            a.cost_center
                            FROM dashboard_gen a 
                            WHERE a.cost_center NOT IN ($cost_center_array2)
                            GROUP BY a.cost_center");
                            $sql_cost_center_selected = mysqli_query($connect, "SELECT 
                            a.cost_center
                            FROM dashboard_gen a 
                            WHERE a.cost_center IN ($cost_center_array2)
                            GROUP BY a.cost_center");
                            ?>
                            <?php while($data_cost_center_selected = mysqli_fetch_assoc($sql_cost_center_selected)){ ?>
                            <option value="<?php echo $data_cost_center_selected['cost_center']; ?>" selected><?php echo $data_cost_center_selected['cost_center']; ?></option>
                            <?php } ?>
                            <?php while($data_cost_center = mysqli_fetch_assoc($sql_cost_center)){ ?>
                ...
                <tr>
                    <td>
                        <select id="status" name="status[]" multiple class="form-control" >
                            ...
                            <option value="nonactive" <?php if($nonactive == 'nonactive'){ echo 'selected'; } ?>>Non Active</option>
                        </select>
                    </td>
                </tr>
            </table>
...
<table width="100%">
    ...
    <tr>
        <td>
            <table width="100%">
                <tr>
                <td width="25%" style="padding-right:2px; padding-top:0px">
            <table width="100%" border="1">
                <tr>
        ...
        <td style="padding-top:1px;">
            <table width="100%" height="100%">
                <tr height = "50%">
                    <td width="50%" style="padding-right:2px; padding-left:2px; ">
                        <table width="100%" border="1">
                            <tr>
