...
<?php  
       ...
       $sql_get_checklistemp = mysqli_query($connect, "SELECT 
       a.code,
       a.name_en
       FROM tgemchecklistgroup a");
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-body">
                            ...
                            <div class="form-row">
                                <div class="col-3 name">Checklist Type * :</div>
                                <div class="col-sm-9" style="padding-left:15px">
                                    <div class="input-group">
                                        <select class="input--style-6" name="checklist_type" id="checklist_type" style="width: ;height: 30px;">
                                            <option value="">Not Specify</option>
                                            <?php 
                                            while($data_reqstatus = mysqli_fetch_assoc($sql_get_checklistemp)){
                                            ?>
                                            <option value="<?php echo $data_reqstatus['code'] ?>"><?php echo $data_reqstatus['name_en']; ?></option>
                                            <?php } ?>
                                        </select>
                                    </div>
                                </div>
                            ...
                            <div class="form-row">
                                <div class="col-3 name">Include Completed :</div>
                                <div class="col-sm-9" style="padding-left:15px">
                                    <div class="input-group" style="margin-top:8px">
                                        <input class="form-check-input" id="include_completed" type="checkbox" value="0" id="flexCheckDefault">
                                    </div>
                                </div>
                            </div>
...
$(document).ready(function(){
        ...
        $(document).on('click', '#view_information', function(){
            ...
            var include_completed = $('#include_completed').val();


            if(checklist_type == ''){
                alert('Checklist type is required!');
                return;
            }
            if(req_employee == ''){
            ...
            $.post('view_report.php',{checklist_type:checklist_type, req_employee:req_employee, include_completed:include_completed}, function (data) {
        ...
        $(document).on('click', '#include_completed', function(){

            var id = $(this).val();
            if(id == 1){
                $('#include_completed').val('0');
            }else if(id == 0){
                $('#include_completed').val('1');
            }
        })
