...
<table class="table table-sm table-bordered" style = "font-size : 12px">
    <?php
        ...
        if(mysqli_num_rows($businessPic) == 0){
            echo '<tr style = "text-align : center"><td style = "padding : 15px">';
            echo '<h6>No data Uploaded</h6>';
            echo '</td></tr>';
        } else {
            echo '
                <tr>
                ...
                while($myPic = mysqli_fetch_array($businessPic)) {
                    echo '<img class="mySlides" src="image/'.$myPic['businessPicture'].'" style="width:100%; height : 100%">';
                }
...
<div class="col-sm-6 name">
<table class="table table-striped table-hover table-sm table-bordered table-responsive" style = "text-align : center; width : 100%">
    <tr style = "text-align : center; background-color : black; color : white">
        ...
        <td> Description </td>
                                                                ...
                                                                <?php
    $sqlProsedurList = "SELECT * FROM hrdprosedurlist WHERE idProsedur = '$nc'";
    $prosedurList = mysqli_query($connect, $sqlProsedurList);
    
        if(mysqli_num_rows($prosedurList) == 0) {
            echo '<tr><td colspan = "4" style = "background-color : #F0FFF0; text-align : left">';
            echo 'No Data';
            echo '</tr></td>';
        } else {
            while($arrayList = mysqli_fetch_array($prosedurList)) {
                echo '<tr>';

                echo '<td>';
                echo $arrayList['list_code'];
                echo '</td>';

                echo '<td style = "text-align : left">';
                echo $arrayList['businessProcess'];
                echo '</td>';
