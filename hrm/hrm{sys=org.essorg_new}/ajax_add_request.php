...
// Mengambil data pos level posisi yang dijadikan parent path
$sql_pos_level          = mysqli_query($connect, "SELECT 
a.pos_level,
a.parent_path
FROM 
hrmorgstruc a
WHERE a.position_id = '$leader_pos'");
$data_pos_level         = mysqli_fetch_assoc($sql_pos_level);
$pos_level              = $data_pos_level['pos_level'] + 1;
$parent_path            = $data_pos_level['parent_path'].','.$leader_pos;
// Mengambil data pos level posisi yang dijadikan parent path
...
// $str_replace[]            = str_replace("%", "'", $data_approval['list_approval']);
