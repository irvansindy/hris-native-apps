...
while ($row = mysqli_fetch_assoc($query)) {
    ...
    // $sql_status_approval    = mysqli_query($connect, "SELECT 
    // a.code,
    // a.name_en
    // FROM hrmstatus a
    // WHERE a.code IN ('1','4','5','3')
    // AND a.code <> '$row[status_approval]'");
    // // Ambil data status approval

    // // Ambil data status approval
    // $sql_status_approval_selected    = mysqli_query($connect, "SELECT 
    // a.code,
    // a.name_en
    // FROM hrmstatus a
    // WHERE a.code = '$row[status_approval]'");
    // // Ambil data status approval
    ...
    // $option = "<select class='input--style-6 approval_status' name='approval_status' id='{$row['request_no']}' style='width:120px ;height: 30px; background:{$bg_color}; color:white'>";

    // while($data_app_sel = mysqli_fetch_assoc($sql_status_approval_selected)){
    //     $option .= "<option value='{$data_app_sel["code"]}' selected>{$data_app_sel["name_en"]}</option>";
    // }

    // while($data_app = mysqli_fetch_assoc($sql_status_approval)){
    //     $option .= "<option value='{$data_app["code"]}' id1='{$row['request_no']}'>{$data_app["name_en"]}</option>";
    // }
    
    // $option .= "</select>";

    $req_status = "<select class='input--style-6 status_req' name='status_req' id='{$row['request_no']}' style='width:120px ;height: 30px; background:{$bg_color_req}; color:black'>";
