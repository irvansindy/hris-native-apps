<?php
    ...
    if ($_POST) {
        ...
        // update status data approval
        $query_update_request_approval = "UPDATE hrmrequestapproval SET
            `request_status` = 1
        WHERE request_no = '$detail_request_no'";

        $exe_update_approval = $connect->query($query_update_request_approval);
        ...
        if ($exe_suggestion_master == FALSE) {
            ...
            $response['messages'] = 'Suggestion failed to be submitted and sent to the approver';
        } else {
            http_response_code(200);
            $response['success'] = true;
            $response['code'] = "success_message";
            $response['messages'] = 'Suggestion successfully submitted and sent to the approver';
        }
