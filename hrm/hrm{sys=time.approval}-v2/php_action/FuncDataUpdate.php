	...
	if($query == TRUE) {
		...
		if($get_any_request['total_approver'] == $get_any_request['total']){
			...
			if($total_leave_balance['total_remaining'] >= $flag_totaldays) {
				...
				} else if($QGet_remaining_id_first_r < $flag_totaldays && $QGet_remaining_id_first_r > 0) {
		...
		if($query == TRUE && $queryq4 == TRUE || $queryq5 == TRUE && $flag_is_deducted_print == "Y"){
		...
		} else if($query == TRUE && $queryq4 == FALSE || $queryq5 == FALSE && $flag_is_deducted_print == "Y"){
			$validator['success'] 	= true;
			$validator['code'] 		= "success_message_approved";
			$validator['messages'] 	= $alert_print_0;
