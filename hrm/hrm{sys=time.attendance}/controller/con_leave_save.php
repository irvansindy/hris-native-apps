...
$get_menu = mysqli_fetch_array(mysqli_query($connect, "SELECT * FROM hrmmenu WHERE menu_id = '8'"));
$get_menu_r = $get_menu['request_type'];
...
$modal_remark           = addslashes($_POST['inp_remark']);
...
$var_daytype_leave     = mysqli_fetch_array(mysqli_query($connect, "SELECT daytype FROM ttamleavetype WHERE leave_code = '$modal_leave'"));
...
$SFMaxReqFrom           = mysqli_fetch_array(mysqli_query($connect, "SELECT 
                                                                  ...
                                                                  AND (SELECT a1.request_status FROM hrmrequestapproval a1
                                                                                    LEFT JOIN hrmleavecancelrequest b1 ON a1.request_no=b1.leaverequest_no
                                                                                    LEFT JOIN hrdleavecancelrequest b2 ON b1.request_no=b2.request_no AND DATE(b2.leave_starttime) = '$modal_leave_end'
                                                                                    WHERE
                                                                                          a1.request_no = b1.leaverequest_no AND 
                                                                                          a1.request_status IN ('8','5')
                                                                                    ORDER BY a1.request_status ASC
                                                                                    LIMIT 1) IS NULL"));
...
$SFMaxReqto             = mysqli_fetch_array(mysqli_query($connect, "SELECT 
                                                                  ...
                                                                  AND DATE(s.leavetime) = '$modal_leave_end' 

                                                                  AND (a.dayrequesttype in ('1','2','3') or a.checkday = '3') 
                                                                  ...
                                                                  AND (SELECT a1.request_status FROM hrmrequestapproval a1
                                                                                    LEFT JOIN hrmleavecancelrequest b1 ON a1.request_no=b1.leaverequest_no
                                                                                    LEFT JOIN hrdleavecancelrequest b2 ON b1.request_no=b2.request_no AND DATE(b2.leave_endtime) = '$modal_leave_end'
                                                                                    WHERE
                                                                                          a1.request_no = b1.leaverequest_no AND 
                                                                                          a1.request_status IN ('8','5')
                                                                                    ORDER BY a1.request_status ASC
...
//VALIDASI TANGGAL PENGAJUAN VS DB CONFIG
$val_leave                          = mysqli_fetch_array(mysqli_query($connect, "SELECT * FROM hrmvalleave WHERE leave_code = '$modal_leave'"));
$val_leave_max_req                  = $val_leave['max_req_day']; //HARI SESUDAH REQUEST
$val_leave_min_req                  = $val_leave_min['min_req_day']; //HARI SEBELUM REQUEST
...
if($val_leave_to_current_print < $val_leave_urg_max_req && $inp_urgent_decl == '1'){
// if($val_leave_to_current_print < $val_leave_max_req && $inp_urgent_decl == '1' && $data_var_validation_allow == '0'){
      echo "<script type='text/javascript'>
                  window.alert('$alert_print3 Max Leave Request Days');
            </script>";
// }else if($val_leave_to_current_print < $val_leave_urg_max_req && $data_var_validation_allow == '0'){
} else if($val_leave_max_req > 0 && $val_leave_to_current_print < $val_leave_max_req && $inp_urgent_decl == '0'){
      echo "<script type='text/javascript'>
                  window.alert('$alert_print3 Max Leave Request Days');     
            </script>";
} else if($val_leave_max_req < 0 && $val_leave_to_current_print < $val_leave_max_req && $inp_urgent_decl == '0'){
...
// VALIDASI APAKAH ADA ISRISAN PENGAJUAN LEAVE
// VALIDASI APAKAH ADA ISRISAN PENGAJUAN LEAVE
//SELAIN DIGUNAKAN UNTUK TOTAL DAYS..QUERY INI JUGA DIGUNAKAN UNTUK SELECT SHIFT
$var_lv_total      = mysqli_fetch_array(mysqli_query($connect, "SELECT
                                                                  a.shiftdaily_code,
                                                                  count(a.daytype) as total,
                                                                  MIN(a.shiftstarttime) AS val_for_starttime,
                                                                  MAX(a.shiftendtime) AS val_for_endtime,
                                                                  CONCAT(DATE(MAX(a.shiftstarttime)), TIME_FORMAT(c.break_starttime, ' %H:%i:%s')) AS break_starttime,
                                                                  CONCAT(DATE(MAX(a.shiftendtime)), TIME_FORMAT(c.break_endtime, ' %H:%i:%s')) AS break_endtime
                                                                        FROM hrdattendance a
                                                                        INNER JOIN view_employee b ON a.emp_id=b.emp_id
                                                                        INNER JOIN hrmttadshiftbreak c ON a.shiftdaily_code=c.shiftdailycode
                                                                              WHERE
                                                                              b.emp_no = '$modal_emp' AND
                                                                              a.daytype IN ('WD','PHWD') AND
                                                                              a.dateforcheck >= '$modal_leave_start' AND 
                                                                              a.dateforcheck <= '$modal_leave_end'"));
...
if($modal_first_type_day == '3' && $inp_leavedaytype == 'HD'){
      ...
      $sequel      = "SELECT 
                                                                              ...
                                                                              AND c.leave_date
                                                                                    between 
                                                                                          '$var_lv_total[break_endtime]' and 
                                                                                          '$var_lv_total[val_for_endtime]'
...
if($modal_first_type_day == '1' && $var_daytype_leave['daytype'] <> 'PD' && $var_daytype_leave['daytype'] <> 'FD'){
      $total_leave = $var_lv_total['total']-0.5;
} elseif($modal_first_type_day == '2' && $modal_first_type_day2 == '1' && $var_daytype_leave['daytype'] <> 'PD' && $var_daytype_leave['daytype'] <> 'FD'){
      $total_leave = $var_lv_total['total']-1;
} elseif($modal_first_type_day == '2' && $modal_first_type_day2 == '3' && $var_daytype_leave['daytype'] <> 'PD' && $var_daytype_leave['daytype'] <> 'FD'){
      $total_leave = $var_lv_total['total']-0.5;
} elseif($modal_first_type_day == '2' && $modal_first_type_day2 == 'undefined' && $var_daytype_leave['daytype'] <> 'PD' && $var_daytype_leave['daytype'] <> 'FD'){
      $total_leave = $var_lv_total['total']-0.5;
} elseif($modal_first_type_day == '3' && $modal_first_type_day2 == '1' && $var_daytype_leave['daytype'] <> 'PD' && $var_daytype_leave['daytype'] <> 'FD'){
      $total_leave = $var_lv_total['total']-0.5;
} elseif($var_daytype_leave['daytype'] == 'PD'){
      $var_productive_hours   = mysqli_fetch_array(mysqli_query($connect, "SELECT productivehours FROM hrmttamshiftdaily WHERE shiftdailycode = '$var_lv_total[shiftdaily_code]'"));
      $var_break_hours        = mysqli_fetch_array(mysqli_query($connect, "SELECT DATE_FORMAT(break_starttime, '%H:%i:%s') as break_starttime,DATE_FORMAT(break_endtime, '%H:%i:%s') as break_endtime FROM hrmttadshiftbreak WHERE shiftdailycode = '$var_lv_total[shiftdaily_code]'"));
      $var_break_hours_start  = $_POST['modal_leave_start']." ".$var_break_hours['break_starttime'];
      $var_break_hours_end    = $_POST['modal_leave_start']." ".$var_break_hours['break_endtime'];

      $var_lv_break_total_dec = mysqli_fetch_array(mysqli_query($connect, "SELECT TIMESTAMPDIFF(MINUTE,'$var_break_hours_start','$var_break_hours_end')/$var_productive_hours[productivehours] as total"));
      $var_lv_total_dec       = mysqli_fetch_array(mysqli_query($connect, "SELECT TIMESTAMPDIFF(MINUTE,'$modal_leave_startTime','$modal_leave_endTime')/$var_productive_hours[productivehours] as total"));

      if($var_lv_total['total'] > '1') {
            $total_leave = $var_lv_total['total'];   
      } else if($inp_starttime == '00:00' || $inp_endtime == '00:00') {
            $total_leave = $var_lv_total['total'];   
      } else {
             if($var_break_hours_start > $modal_leave_startTime && $var_break_hours_start < $modal_leave_endTime || $var_break_hours_end > $modal_leave_startTime && $var_break_hours_end < $modal_leave_endTime)
                  {
                        $total_leave            = $var_lv_total_dec['total']-$var_lv_break_total_dec['total'];
                        $dataon = '1';
                  } else {
                        $total_leave            = $var_lv_total_dec['total'];
                        $dataon = '2';
            }
      }
...
if($get_approver_setting_sf > 0){
      $modal_approval            = mysqli_query($connect, 
            ...
            if(mysqli_num_rows($modal_approval) == '0') {
                  echo "<script type='text/javascript'>
                              window.alert('Wrong approval setting'); 
                        </script>";
            }
} else {
...
if($process_approval) 
                  ...
                  $leave_request_detail            = mysqli_query($connect, "SELECT 
                                                ...
                                                while($r_detail=mysqli_fetch_array($leave_request_detail)){
                                                      ...
                                                      if(!empty($inp_starttime) && $inp_starttime != '00:00')
                                                      {
                                                            ...
                                                            if($inp_leavedaytype_print == '2') {
                                                                  $get_shift_breaks             = mysqli_fetch_assoc(mysqli_query($connect, "SELECT TIME_FORMAT(a.break_endtime, '%H:%i:%s') as break_endtime FROM hrmttadshiftbreak a WHERE a.shiftdailycode = '$var_lv_total[shiftdaily_code]'"));
                                                                  $SFdatetimeStart              = date("$SFdatetimeConv $get_shift_breaks[break_endtime]");
                                                            } else {
                                                                  $SFdatetimeStart              = $r_detail['shiftstarttime'];
                                                            }
                                                      }
                                                      ...
                                                      if(!empty($inp_endtime) && $inp_endtime != '00:00')
                                                      {
                                                            ...
                                                            if($inp_leavedaytype_print == '1') {
                                                                  $get_shift_breaks             = mysqli_fetch_assoc(mysqli_query($connect, "SELECT TIME_FORMAT(a.break_starttime, '%H:%i:%s') as break_starttime FROM hrmttadshiftbreak a WHERE a.shiftdailycode = '$var_lv_total[shiftdaily_code]'"));
                                                                  $SFdatetimeEnd                = date("$SFdatetimeConv $get_shift_breaks[break_starttime]");
                                                            } else {
            ...
            {
                  echo"<script type='text/javascript'>
                              window.alert('Something went wrong, failed to submit leave request');     
                        </script>";


                        echo "INSERT INTO hrmrequestapproval 
                                                      (
                                                            `request_no`,
                                                            `approval_list`,
                                                            `seq_id`,
                        ...
                        mysqli_query($connect, "DELETE FROM hrmleaverequest WHERE request_no='$SFnumbercon'");
                        mysqli_query($connect, "DELETE FROM hrdleaverequest WHERE request_no='$SFnumbercon'");
                        mysqli_query($connect, "DELETE FROM hrmrequestapproval WHERE request_no='$SFnumbercon'");
            } 
      }
