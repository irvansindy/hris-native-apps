...
<table id="datatable" width="99%" class="table table-bordered table-striped table-hover table-head-fixed">
        ...
        <?php while ($row = mysqli_fetch_array($data)) { ?>
            <tr>
                <td style="cursor: pointer;" onclick="SetName<?php echo $row['word'] ?>();">
                <input type="hidden" 
                        name="ddlNames" 
                        ...
                        style="width: 98%;">
                    <?php echo $row['word'] ?><?php echo $git; ?>
               
                
                <?php echo "<script type='text/javascript'>
                function SetName$row[word]() {
                        if (window.opener != null && !window.opener.closed) {
                                var formula$git = window.opener.document.getElementById('formula$git');
                                formula$git.value = '$datax' + ' {' + document.getElementById('ddlNames$row[word]').value + '}';
                        }
                        window.close();
                }
                </script>";
