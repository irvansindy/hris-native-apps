...
if($_POST) {	
	...
	for($iemg=0;$iemg<count($_POST['FactorHour']);$iemg++){
		...
		$SelOTminutes 	= $_POST['SelOTminutes'][$iemg];
		$SelOTmeal 		= $_POST['SelOTmeal'][$iemg];
		$SelOTtransport 	= $_POST['SelOTtransport'][$iemg];
	...
	for($iemg=0;$iemg<count($_POST['SelOTtypeother']);$iemg++){
		$iemg_plus 		= $iemg+1;
		$SelOTtypeother	= $_POST['SelOTtypeother'][$iemg];
		$SelOTminutes		= $_POST['SelOTminutes'][$iemg];
		$SelOTmeal 		= $_POST['SelOTmeal'][$iemg];
		$SelOTtransport 	= $_POST['SelOTtransport'][$iemg];

		if($SelOTtypeother == 'OTMEAL'){
			$value 	= $SelOTmeal;
		} else {
			$value 	= $SelOTtransport;
		}
