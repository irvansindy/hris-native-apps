...
<table class="table table-hover table-bordered table-head-fixed table-striped" id="tbl_posts">
       ...
       <?php
                                                                      ...
                                                                      $modal=mysqli_query($connect, "SELECT
                                                                                                         ...
                                                                                                         FROM
                                                                                                         hrmperf_target a
                                                                                                         LEFT JOIN hrmperf_iprecord b ON a.reference = b.document_reference AND b.kpi_perspektif_id = '$sfid' AND b.ipp_reqno = '$rfid' AND b.document_reference = 'appraisal.effident' AND a.target_name = b.target AND b.ipp_id = '$tfid'
       ...
       <tbody id="tbl_posts_body">
                     ...
                     $(document).on("change", "#file<?php echo $row_performance['kpi_pers_spv_up']; ?><?php echo $row_performance['target_name']; ?>",
                            function() {
                                   var name = document.getElementById("file<?php echo $row_performance['kpi_pers_spv_up']; ?><?php echo $row_performance['target_name']; ?>").files[0].name;
                                          ...
                                          $.ajax({
                                                 url: "uploader/index.php?&code=22&&token=<?php echo $sfid; ?><?php echo $row_performance['target_name']; ?>&req_no=<?php echo $rfid; ?>&target=<?php echo $row_performance['target_name']; ?>&period=<?php echo $row_performance['period_id']; ?>&ipp_id=<?php echo $tfid; ?>",
                                                 method: "POST",
                                                 data: form_data,
                                                 contentType: false,
                                                 cache: false,
              ...
              <script>
                     ...
                     type: 'post',
                     data: { field: remark<?php echo $row_performance['kpi_pers_spv_up']; ?><?php echo $row_performance['target_name']; ?> , req_no: '<?php echo $rfid; ?>' , target : '<?php echo $row_performance['target_name']; ?>' , ipp_id : '<?php echo $tfid; ?>' , kpi_perspektif_id : '<?php echo $sfid; ?>'},
