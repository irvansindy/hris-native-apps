...
<?php while ($row = mysqli_fetch_array($get_Formula)) { ?>
              ...
              $var2 = array(
                                   ...
                                   "(SELECT `hrmttamshiftdaily`.productivehours FROM `hrdattendance` LEFT JOIN `hrmttamshiftdaily` ON  `hrdattendance`.shiftdaily_code=`hrmttamshiftdaily`.shiftdailycode WHERE `attend_id`='$get_attendance_attend_id')",
                                   "a.shiftdaily_code",
                                   "<> '0000-00-00 00:00:00'",
                                   "'",
                                   "')"
       ...
       $connect->query("DELETE FROM `hrdattstatusdetail` WHERE 
                            `attend_id`='$get_attendance_attend_id' AND
                            `attend_code` IN ('$row[listattendcode]')");
