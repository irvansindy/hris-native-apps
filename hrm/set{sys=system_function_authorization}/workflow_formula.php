...
<?php
	$att_formula = "SELECT * FROM view_employee a 
		LEFT JOIN hrmorgstruc h on a.emp_no=h.emp_no
		WHERE a.emp_no = '$employee' AND $conversion_formula";
	...
	if ($formula_process > 0) {
			...
			$sql_approval = "INSERT IGNORE INTO hrmrequestapproval 
				(
					`request_no`,
					`request_type`,
					`approval_list`,
			...
			mysqli_query($connect, "INSERT IGNORE INTO hrmrequestapproval 
				(
					`request_no`,
					`request_type`,
					`approval_list`,
