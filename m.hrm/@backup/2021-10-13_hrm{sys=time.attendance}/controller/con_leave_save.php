...
$inp_leavecompany       = $_POST['inp_leavecompany'];
 ...
 * --------------- By Display:inline ------------- '''' -----------
 */
//VALIDASI APAKAH KARYAWAN MEMILIKI PERSETUJUAN DARI HR PLANT
//VALIDASI APAKAH KARYAWAN MEMILIKI PERSETUJUAN DARI HR PLANT
$val_allow_leave               = mysqli_query($connect, "SELECT emp_id,request_access FROM hrdvalleave WHERE emp_id = '$modal_emp' and `status` = 'Active'");
$val_allow_leave_num_rows      = mysqli_num_rows($val_allow_leave);
$val_allow_leave_num_res      = mysqli_fetch_array($val_allow_leave);

if($val_allow_leave_num_rows > 0){
      $val_allow_leave_print_1       = $val_allow_leave_num_res['emp_id'];
      $val_allow_leave_print_2       = $val_allow_leave_num_res['request_access'];
}
//VALIDASI APAKAH KARYAWAN MEMILIKI PERSETUJUAN DARI HR PLANT
//VALIDASI APAKAH KARYAWAN MEMILIKI PERSETUJUAN DARI HR PLANT
...
if($val_leave_to_current_print < $val_leave_max_req && $inp_urgent_decl == '1' && $val_allow_leave_num_rows < 1){
      echo "<script type='text/javascript'>
...
}else if($val_leave_to_current_print < $val_leave_urg_max_req && $val_allow_leave_num_rows < 1){
      echo "<script type='text/javascript'>
...
$val_leave_balance   = mysqli_fetch_array(mysqli_query($connect, "SELECT 
                                                                        ...
                                                                        REPLACE(c.remaining,'.0000','') as remaining
...
$conversion_2 = str_replace($var1, $var2, $alert_print2 );

if($val_leave_remaining_balance_print < $total_leave && $val_allow_leave_num_rows < 1 && $inp_leavecompany == 'N'){
      echo "<script type='text/javascript'>
                  window.alert('$conversion_2');     
            </script>";
} else {
//VALIDASI APAKAH LEAVE BALANCE AKTIF CUKUP
//VALIDASI APAKAH LEAVE BALANCE AKTIF CUKUP
...
if($total > 0){
      ...
      if($printf > 0)
            ...
            //APABILA REQUEST DILAUKAN OLEH SEORANG APPROVER
            //APABILA REQUEST DILAUKAN OLEH SEORANG APPROVER
            ...
            //APABILA REQUEST INI VIA MEMO HR
            //APABILA REQUEST INI VIA MEMO HR
            if ($val_allow_leave_num_rows == '1') {
                  $inp_allow_request_setting_hr = 'Y';
                  $inp_request_access_code = $val_allow_leave_print_2;
            } else {
                  $inp_allow_request_setting_hr = 'N';
                  $inp_request_access_code = NULL;
            }
            //APABILA REQUEST INI VIA MEMO HR
            //APABILA REQUEST INI VIA MEMO HR
                                    ...
                                    (
                                          ...
                                          `token`) 
                                                ...
                                                (
                                                      ...
                                                      '$inp_allow_request_setting_hr',
                                                      '$inp_request_access_code',
                                                      '$SFGet_token')
                                    ");
