...
// MENDAPATKAN TANGGAL DIBUAT------------------------------>
$get_leave_startdate   			= mysqli_fetch_array(mysqli_query($connect, "SELECT leave_code FROM hrmleavecancelrequest WHERE request_no='$modal_id_leaverequest'"));
...
$get_emp   						= mysqli_fetch_array(mysqli_query($connect, "SELECT a.emp_id
														...
														WHERE a.request_no = '$val_leavereq_print'"));
...
//                   window.alert('$alert_print4 Max Approve Request is $val_leave_max_approved Days');     
//             </script>";
// } else {
//VALIDASI TANGGAL PENGAJUAN VS DB CONFIG
//VALIDASI TANGGAL PENGAJUAN VS DB CONFIG
...
$var_check_sum_approver        = $check_sum_approver['total']+1;
	...
	if($check_count_required['total'] > 1){
		...
		if($process_frs && $process_sec && $req_code == '3')
		{
				...
				// PROSES MENAMBAHKAN LEAVE BALANCE KE ttadempgetleave ----->
				$getleave			= mysqli_fetch_array(mysqli_query($connect, "SELECT SUM(remaining) as remaining
																FROM hrmempleavebal 
															WHERE 
															emp_id = '$get_emp_print' and 
															leave_code = '$get_leave_type_print'"));
				$getleave_print 		= $getleave['remaining'];
				
				$getprint 			= "UPDATE hrmempleavebal SET remaining = '$remaining_balance', remark='sys proc $remaining_balance' 
				WHERE 
				emp_id = '$get_emp_print' and
				leave_code = '$get_leave_type_print' and
				(SELECT deductedleave FROM ttamleavetype WHERE leave_code = '$get_leave_type_print') = 'Y'";
				...
				if($flag_is_deducted_print == 'Y') {
					$process_back_leave 	= mysqli_query($connect, "UPDATE hrmempleavebal SET remaining = '$remaining_balance', remark='sys proc $remaining_balance' 
												WHERE 
												emp_id = '$get_emp_print' and
												leave_code = '$get_leave_type_print' and
												(SELECT deductedleave FROM ttamleavetype WHERE leave_code = '$get_leave_type_print') = 'Y'");
					if($process_back_leave ){
						echo "<script type='text/javascript'>
							window.alert('Successfully Approved Request [ Fully Approved ] $remaining_balance');
							window.location.replace('../hrm{sys=leave.cancellation.approval}?emp_id=$username');
						</script>";
					} else {
						echo "<script type='text/javascript'>
