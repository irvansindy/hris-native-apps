...
$var_check_sum_approver        = $check_sum_approver['total']+1;
	...
	if($check_count_required['total'] > 1){
		echo "<script type='text/javascript'>
			...
			window.location.replace('../hrm{sys=time.approval}?emp_id=<?php echo $username; ?>');       
		</script>";
		...
		if($process_frs && $process_sec && $req_code == '3')
		{
				...
				$process_back_leave 		= mysqli_query($connect, "UPDATE hrmempleavebal SET remaining='$remaining_balance', remark='agus proc $remaining_balance' WHERE emp_id = '$get_emp_print '");
				// PENAMBAHAN TOTAL CANCEL DITAMBAH SALDO REMAINING BALANCE ->
				// PENAMBAHAN TOTAL CANCEL DITAMBAH SALDO REMAINING BALANCE ->
