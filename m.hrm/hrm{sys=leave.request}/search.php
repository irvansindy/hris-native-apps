...
if (isset($_POST["query"])) {
  ...
  $query = mysqli_query($connect, "SELECT
                                      ...
                                      SELECT
                                            ...
                                            LEFT JOIN 
                                                    (
                                                      SELECT * FROM view_employee WHERE cost_code LIKE '%GHR%'
                                                    ) b ON a.request_type = 'Attendance.leave'
                                            WHERE 
                                              (a.empno_appvr1 LIKE '%$key0%' OR
                                              a.empno_appvr2 LIKE '%$key0%' OR
                                              a.empno_appvr3 LIKE '%$key0%') AND
                                              (b.emp_no LIKE '$key%' OR b.Full_Name LIKE '$key%') AND
                                              (b.end_date IS NULL OR b.end_date = '0000-00-00 00:00:00')
                                        GROUP BY b.emp_no
