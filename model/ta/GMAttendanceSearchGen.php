       ...
       $SFdate                 = date("Y-m-d");
		...
		$req_app_use_full_query	= mysqli_query($connect, "SELECT
			...
			if (!empty($_POST['inp_emp']) && !empty($_POST['inp_name']) && !empty($_POST['inp_date']) && !empty($_POST['inp_enddate'])) {
					...
					$identitydate        = $_POST['inp_date'];
					$identityenddate     = $_POST['inp_enddate'];
					$where               = "WHERE (a.emp_no like '%$identitynip%') AND (a.Full_Name like '%$identityname%') AND (b.dateforcheck >= '$identitydate' AND b.dateforcheck <= '$identityenddate') AND ($conversion)";
			} elseif (!empty($_POST['inp_emp']) && !empty($_POST['inp_date']) && !empty($_POST['inp_enddate']) ) {
					$identitynip         = $_POST['inp_emp'];
					$identitydate        = $_POST['inp_date'];
					$identityenddate     = $_POST['inp_enddate'];
					$where               = "WHERE (a.emp_no like '$identitynip') AND (b.dateforcheck >= '$identitydate') AND b.dateforcheck <= '$identityenddate') AND ($conversion)";
			} elseif (empty($_POST['inp_emp']) && !empty($_POST['inp_date']) && !empty($_POST['inp_enddate']) ) {
					$identitynip         = $_POST['inp_emp'];
					$identitydate        = $_POST['inp_date'];
					$identityenddate     = $_POST['inp_enddate'];
					$where               = "WHERE (b.dateforcheck >= '$identitydate' AND b.dateforcheck <= '$identityenddate') AND ($conversion)";
					...
					$identitydate        = $_POST['inp_date'];
					$where               = "WHERE b.dateforcheck = '$identitydate' AND ($conversion)";
			} else {
					$where               = "WHERE ($conversion) AND b.dateforcheck = '$SFdate'";
